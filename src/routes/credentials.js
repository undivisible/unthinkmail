...
export async function handleCredentials(request, env) {
  ...
  if (request.method === 'PUT') {
    ...
    for (const field of REQUIRED_FIELDS) {
      if (body[field] == null) return jsonError(`Missing required field: ${field}`, 400);
    }
    const credentialsObj = {};
    for (const field of REQUIRED_FIELDS) credentialsObj[field] = body[field];
  ...
  if (request.method === 'GET') {
    ...
    if (!record) return jsonError('No credentials stored', 404);
    ...
    return json({
      ...
      smtp_host: decrypted.smtp_host,
      smtp_port: decrypted.smtp_port,
    });
  }

  if (request.method === 'DELETE') {
    const deleted = await deleteCredentials(env.DB, user.userId);
    if (!deleted) return jsonError('No credentials stored', 404);
